...
%   Using stochastic motion and measurement models, localize the robot
%   based on a discrete Bayes filter. Here we use the Odometry Motion Model
%   and the Range Finder Measurement Model.
%
%   Remember that:
...
%       measurement_model = p(zt | xt, m)
    ...
    for k = 1:count % line 2 of Table 8.1
        ...
        pkt(rowk,colk,depthk) = pbarkt(rowk,colk,depthk)*p(k,1);
        % -----------------------------------------------------------------
...
%   See Table 5.7, p. 141
    ...
    if is_occupied(xt, m)
        ...
        % how many spots could robot be?
        N = count - obstacle_count;
        
        % If it's equally likely for robot to be in a non-obstacle spot,
